...
public Plugin myinfo = {
	name = "[CS:GO] Competitive Ranks/Coins",
...
public void OnPluginStart()
{
	...
	LoadTranslations("valveranksandcoins.phrases");
	
	g_hCvarVersion = CreateConVar("sm_valveranksandcoins_version", PLUGIN_VERSION, "Fake rank version", FCVAR_SPONLY|FCVAR_REPLICATED|FCVAR_NOTIFY|FCVAR_DONTRECORD);
	g_hCvarVersion.SetString(PLUGIN_VERSION);
	g_ShowCoins = CreateConVar("sm_valveranksandcoins_coins", "1", "Show legit coins on players, if enabled", _, true, 0.0, true, 1.0);
	g_ShowProfileRanks = CreateConVar("sm_valveranksandcoins_profileranks", "1", "Show legit profile ranks on players, if enabled", _, true, 0.0, true, 1.0);
	...
	AutoExecConfig(true, "valveranksandcoins");
...
public void OnMapStart()
{
	...
	BuildPath(Path_SM, configFile, sizeof(configFile), "configs/valveranksandcoins.cfg");

	if (!FileExists(configFile))
	{
		LogError("The valveranksandcoins config (%s) file does not exist", configFile);
		return;
	}

	kv = CreateKeyValues("valveranksandcoins");
	FileToKeyValues(kv, configFile);
	if (!KvGotoFirstSubKey(kv))
	{
		LogError("The valveranksandcoins config (%s) file was empty", configFile);
		return;
	}
